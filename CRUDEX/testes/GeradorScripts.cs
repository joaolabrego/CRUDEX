    ...
    public class ScriptsSql
    {
        ...
        private static StringBuilder GetScriptReadTable(DataRow table, TDataRows columns, TDataRows domains, TDataRows types)
        {
            ...
            if (columnRows.Count > 0)
            {
                ...
                result.Append($"                IF @S_Id IS NOT NULL BEGIN\r\n");
                result.Append($"                    SET @Where = @Where + CASE WHEN @Where = '' THEN '' ELSE ' AND ' END + '[T].[Id] = @Id'\r\n");
                result.Append($"                END\r\n");
                result.Append($"                SET @sql =';WITH CTE AS (SELECT ROW_NUMBER() OVER (ORDER BY ' + @OrderBy + ') AS Recno,\r\n");
                result.Append($"                                                [T].[Id]\r\n");
                result.Append($"                                            FROM [#tmpTable] [#]\r\n");
                result.Append($"                                                INNER JOIN [dbo].[Masks] [T] ON [T].[Id] = [#].[Id]\r\n");
                result.Append($"                                            WHERE [#].[_] = ''T''\r\n");
                result.Append($"                                         UNION ALL\r\n");
                result.Append($"                                         SELECT ROW_NUMBER() OVER (ORDER BY ' + @OrderBy + ') + (SELECT COUNT(*) FROM [#tmpTable] [#] WHERE [#].[_] = ''T''),\r\n");
                result.Append($"                                                [O].[Id]\r\n");
                result.Append($"                                            FROM [#tmpTable] [#]\r\n");
                result.Append($"                                                INNER JOIN [#tmpOperations] [O] ON [O].[Id] = [#].[Id]\r\n");
                result.Append($"                                            WHERE [#].[_] = ''O'')\r\n");
                result.Append($"                                SELECT TOP 1 @PageNumber = (Recno - 1) / @LimitRows + 1\r\n");
                result.Append($"                                    FROM CTE\r\n");
                result.Append($"                                        INNER JOIN [dbo].[Masks] [T] ON [T].[Id] = [CTE].[Id]' +\r\n");
                result.Append($"                          CASE WHEN @Where = '' THEN '' ELSE ' WHERE' + @Where END\r\n");
                result.Append($"                EXEC sp_executesql @sql\r\n");
                result.Append($"            END\r\n");
                result.Append($"        END\r\n");
                firstTime = true;
