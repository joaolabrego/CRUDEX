...
namespace CRUDA_LIB
{
    public class Config
    {
        ...
        public Config(string systemName, string? databaseName = null, string? tableName = null)
        {
            ...
            {
                CryptoKey = Crypto.GenerateCryptoKey();
                Data = new
                {
                    RowsPerPage = Convert.ToInt32(Settings.Get("ROWS_PER_PAGE")),
                    IdleTimeInMinutesLimit = Convert.ToInt32(Settings.Get("IDLE_TIME_IN_MINUTES_LIMIT")),
                    PaddingBrowseLastPage = Convert.ToBoolean(Settings.Get("PADDING_GRID_LAST_PAGE")),
