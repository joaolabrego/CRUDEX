...
GO
IF(SELECT object_id('[dbo].[CommitAll]', 'P')) IS NULL
	EXEC('CREATE PROCEDURE [dbo].[CommitAll] AS PRINT 1')
GO
ALTER PROCEDURE[dbo].[CommitAll](@TransactionId BIGINT = NULL) AS BEGIN
	BEGIN TRY
		SET NOCOUNT ON
		SET TRANSACTION ISOLATION LEVEL READ COMMITTED

		DECLARE @ErrorMessage VARCHAR(255) = 'Stored Procedure CommitAll: '
		...
		END
		IF @@TRANCOUNT = 0
			BEGIN TRANSACTION CommitAll
		ELSE
			SAVE TRANSACTION CommitAll
		...
		COMMIT TRANSACTION CommitAll

		RETURN 1
	END TRY
	BEGIN CATCH
		ROLLBACK TRANSACTION CommitAll;
		THROW
	END CATCH
END
